...
describe('ListAnimePage', () => {
  ...
  describe('get list anime', () => {

    animeService.listAnime().subscribe((listOfMovies:any) => {
        expect(listOfMovies).toEqual(mockListAnime);
        expect(listOfMovies.data.length).toBe(mockListAnime.data.length);
        expect(listOfMovies.data.length).not.toBe(20);
      });

      const httpMock = httpController.expectOne(animeService.url);
      httpMock.flush(mockListAnime);
  });
})